...
def manhattan_pro(game_map, a, b):
    if a == b:
        return 1
    if 1:
        pass
...
class DroneProblem(search.Problem):
    ...
    def __init__(self, initial):
        ...
        # updates the map
        self._where_I = self._update_map(game_map, packages)
        ...
        coordinate_to_packages = self.get_packages_at_coordinate(packages)
        # -------- drones data --------
        # drones packages
        ...
        # saves the initial state for planning better heuristic later
        self.root = state
    ...
    def goal_test(self, state):
        ...
        state = json.loads(state)
        return sum([num for num in state['client_packages_counter'].values()]) == 0
    ...
    def create_package_name_to_client_name(self, clients):
        """
    ...
    def drone_can_deliver_package(self, clients, package_name, x, y, turns):
        """
        ...
        """
        client_name = self.create_package_name_to_client_name(clients)[package_name]
    ...
    def drones_action_builder(self, state):
        ...
        for drone_name, coordinate in drones:
            ...
            for pack_name in state['drones_packages'][drone_name]:
                ...
                client_name = self.drone_can_deliver_package(state['clients'], pack_name, x, y, state['turns'])
    ...
    def get_packages_at_coordinate(self, packages):
        """

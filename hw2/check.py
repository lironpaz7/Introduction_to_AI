    ...
    def is_atomic_action_legal(self, atomic_action, drones_already_moved):
        ...
        if action_name == "move":
            ...
            try:
                ...
                if destination[0] < 0 or destination[1] < 0 or destination[0] >= len(self.state["map"]) or destination[
                    1] >= len(self.state["map"][0]):
    ...
    def environment_step(self):
        ...
        for client, properties in self.state["clients"].items():
            for _ in range(1000):
                ...
                if new_coordinates[0] < 0 or new_coordinates[1] < 0 or new_coordinates[0] >= len(self.state["map"]) or \
                        new_coordinates[1] >= len(self.state["map"][0]):
                    continue
                break
            else:
